...
export function ProtocolStatusButton({ id, status }: { id?: string; status?: string | null }) {
  ...
  async function complete() {
    ...
    setLoading(false);
    if (!response.ok) {
      setError(data.error ?? copy.protocols.updateError);
      return;
    }
    router.refresh();
  }
  ...
  if (status === "completed") {
    return <span className="rounded-md border border-emeraldx/30 px-3 py-2 text-sm text-emeraldx">{copy.common.completed}</span>;
  }

  return (
    <div>
      <Button onClick={complete} disabled={loading || !id} className="bg-white/10 text-white hover:bg-white/15">
        {loading ? copy.common.updating : copy.protocols.markCompleted}
      </Button>
