...
import type { BiologicalInsightRecord, LabReport } from "@/types/database";
...
export default async function LabsPage() {
  ...
  const [{ data, error }, { data: labInsights }] = await Promise.all([
    supabase.from("lab_reports").select("*").eq("user_id", auth.user.id).order("created_at", { ascending: false }).returns<LabReport[]>(),
    supabase.from("biological_insights").select("*").eq("user_id", auth.user.id).eq("status", "active").in("source_type", ["lab", "missing_data"]).order("created_at", { ascending: false }).limit(6).returns<BiologicalInsightRecord[]>()
  ]);
  const reports = data ?? [];
  ...
  return (
    <div className="space-y-6">
      ...
      {(labInsights?.length ?? 0) > 0 && <Card><div className="flex items-center justify-between gap-3"><div><p className="system-label">{copy.dashboard.intelligenceLayer}</p><h3 className="mt-2 font-semibold text-white">{copy.dashboard.activeBiologicalInsights}</h3></div><SystemStatus label={copy.dashboard.online} tone="intelligence" /></div><div className="mt-5 grid gap-3 md:grid-cols-2">{(labInsights ?? []).slice(0, 4).map((insight) => <div key={insight.id} className="command-surface rounded-md p-4 text-sm"><p className="system-label">{localizePillar(String(insight.pillar), language)} | {insight.severity}</p><p className="mt-2 font-semibold text-white">{insight.title}</p><p className="mt-2 leading-6 text-chrome">{insight.summary}</p>{Array.isArray(insight.recommended_actions) && insight.recommended_actions[0] ? <p className="mt-3 text-emeraldx">{String(insight.recommended_actions[0])}</p> : null}</div>)}</div></Card>}
