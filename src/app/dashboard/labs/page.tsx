...
import { localizeLabCategory, localizeLabPriorityAction, localizeLabStatus } from "@/lib/i18n";
    ...
    <div className="space-y-6">
      ...
      <section className="grid gap-4 lg:grid-cols-[420px_1fr]">
        ...
        <Card>
          ...
          {latest ? <div className="mt-4 space-y-5"><div className="grid gap-3 sm:grid-cols-3"><div className="rounded-md bg-white/5 p-4"><p className="text-xs uppercase text-chrome">{copy.labs.markers}</p><p className="mt-2 text-2xl text-white">{latest.biomarkers.length}</p></div><div className="rounded-md bg-white/5 p-4"><p className="text-xs uppercase text-chrome">{copy.labs.opportunities}</p><p className="mt-2 text-2xl text-white">{latest.biggestOpportunities.length}</p></div><div className="rounded-md bg-white/5 p-4"><p className="text-xs uppercase text-chrome">{copy.labs.weakest}</p><p className="mt-2 text-lg text-emeraldx">{localizeLabCategory(latest.weakestCategory, language) ?? copy.common.none}</p></div></div><div><p className="text-xs uppercase tracking-[0.18em] text-emeraldx">{copy.labs.priority}</p><ul className="mt-3 space-y-2 text-sm text-chrome">{latest.biggestOpportunities.length ? latest.biggestOpportunities.slice(0, 3).map((marker) => <li key={marker.key}>{localizeLabPriorityAction(marker.name, language)}</li>) : <li>{copy.optimization.labs.maintain}</li>}</ul></div></div> : <p className="mt-4 text-sm text-chrome">{copy.labs.noAnalysis}</p>}
        </Card>
      </section>
