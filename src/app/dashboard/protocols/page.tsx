            ...
            {details ? (
              <div className="mt-6 space-y-5 border-t border-signal/10 pt-5">
                <div className="grid gap-3 md:grid-cols-4">
                  ...
                  <div className="command-surface rounded-md p-4"><Activity className="h-4 w-4 text-warningx" /><p className="mt-3 system-label">{copy.protocols.estimatedImpact}</p><p className="mt-2 text-white">{details.expectedImpact ?? (details.intensity === "Advanced" ? copy.protocols.highImpact : copy.protocols.moderateImpact)}</p>{details.confidenceLevel ? <p className="mt-1 text-[10px] text-emeraldx">{details.confidenceLevel}</p> : null}</div>
                ...
                {(details.biologicalRationale || details.contextSummary?.length > 0) && <div className="rounded-md border border-violetx/20 bg-violetx/[0.045] p-4"><h4 className="system-label text-violet-200">{copy.protocols.rationale}</h4><p className="mt-3 text-sm leading-6 text-chrome">{details.biologicalRationale ?? details.contextSummary.slice(0, 3).map((item) => localizeProtocolText(item, language)).join(" | ")}</p></div>}
